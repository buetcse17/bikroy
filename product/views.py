    ...
    if id == 1:
        ...
        for r in result:
            ...
            row = {'product_name': product_name, 'device_catagory': device_catagory,
                'price': price, 'condition': condition,'product_id':product_id, 'product_image':product_image}
        ...
        for r in result:
            ...
            row = {'product_name': product_name,
                'pet_type': pet_type, 'price': price,'product_id':product_id,'product_image':product_image}
            dict_result.append(row)
    elif id == 3:
        cursor = conn.cursor()
        ...
        for r in result:
            ...
            product_image = r[5]
            row = {'product_name': product_name,
                'genre': genre, 'price': price,'condition': condition,'product_id':product_id, 'product_image':product_image}
            dict_result.append(row)
    elif id == 4:
        cursor = conn.cursor()
        ...
        for r in result:
            ...
            product_image = r[4]
            row = {'product_name': product_name,
                'price': price,'course_title': course_title,'product_id':product_id, 'product_image':product_image}
            dict_result.append(row)
    elif id == 5:
        cursor = conn.cursor()
        ...
        for r in result:
            ...
            product_image = r[5]
            row = {'product_name': product_name,
                'tutor_gender': tutor_gender, 'price': price,'education_level': education_level,'product_id':product_id, 'product_image':product_image}
            dict_result.append(row)
...
def listProductAreaWise(request, id, area):
    ...
    if id == 1:
        cursor = conn.cursor()
        sql = """   SELECT p.PRODUCT_NAME, d.DEVICE_CATAGORY, price, CONDITION, p.product_id , im.IMAGE_URL 
                    FROM product p, DEVICES d, ADVERTISEMENT ad, location loc, account ac, profile pf , image im 
                    WHERE d.PRODUCT_ID=p.PRODUCT_ID and ad.ADVERTISEMENT_ID=p.ADVERTISEMENT_ID and ad.ADVERTISEMENT_TYPE='paid' and im.PRODUCT_ID=p.PRODUCT_ID
        ...
        cursor = conn.cursor()
        sql = """   SELECT pr.PRODUCT_NAME,p.PET_TYPE,price,pr.product_id , im.image_url 
                    FROM product pr,pet p,ADVERTISEMENT ad, location loc, account ac, profile pf , image im 
                    WHERE pr.PRODUCT_ID=p.PRODUCT_ID and ad.ADVERTISEMENT_ID=pr.ADVERTISEMENT_ID and ad.ADVERTISEMENT_TYPE='paid' and im.PRODUCT_ID=pr.PRODUCT_ID 
                    and pf.LOCATION_ID=loc.LOCATION_ID AND ac.USERNAME=ad.USERNAME AND ac.PROFILE_NO=pf.PROFILE_NO AND LOWER(loc.DIVISION)=:area
                    order by ad.AD_TIME desc
                """
        ...
        cursor = conn.cursor()
        sql = """   SELECT pr.PRODUCT_NAME,b.genre,condition, price,pr.product_id , im.image_url 
                    FROM product pr,book b,ADVERTISEMENT ad , location loc, account ac, profile pf , image im 
                    WHERE pr.PRODUCT_ID=b.PRODUCT_ID and ad.ADVERTISEMENT_ID=pr.ADVERTISEMENT_ID and ad.ADVERTISEMENT_TYPE='paid' and im.PRODUCT_ID=pr.PRODUCT_ID 
                    and pf.LOCATION_ID=loc.LOCATION_ID AND ac.USERNAME=ad.USERNAME AND ac.PROFILE_NO=pf.PROFILE_NO AND LOWER(loc.DIVISION)=:area
                    order by ad.AD_TIME desc
                """
        ...
        cursor = conn.cursor()
        sql = """   SELECT pr.PRODUCT_NAME,COURSE_TITLE,price,pr.product_id , im.image_url  
                    FROM product pr,course c,ADVERTISEMENT ad , location loc, account ac, profile pf , image im
                    WHERE pr.PRODUCT_ID=c.PRODUCT_ID and ad.ADVERTISEMENT_ID=pr.ADVERTISEMENT_ID and ad.ADVERTISEMENT_TYPE='paid'  and im.PRODUCT_ID=pr.PRODUCT_ID 
                    and pf.LOCATION_ID=loc.LOCATION_ID AND ac.USERNAME=ad.USERNAME AND ac.PROFILE_NO=pf.PROFILE_NO AND LOWER(loc.DIVISION)=:area
                    order by ad.AD_TIME desc
                """
        ...
        cursor = conn.cursor()
        sql = """   SELECT pr.PRODUCT_NAME,TUTOR_GENDER,EDUCATION_LEVEL,price,pr.product_id , im.image_url 
                    FROM product pr,tution t,ADVERTISEMENT ad, location loc, account ac, profile pf , image im
                    WHERE pr.PRODUCT_ID=t.PRODUCT_ID and ad.ADVERTISEMENT_ID=pr.ADVERTISEMENT_ID and ad.ADVERTISEMENT_TYPE='paid' and im.PRODUCT_ID=pr.PRODUCT_ID 
                    and pf.LOCATION_ID=loc.LOCATION_ID AND ac.USERNAME=ad.USERNAME AND ac.PROFILE_NO=pf.PROFILE_NO AND LOWER(loc.DIVISION)=:area
                    order by ad.AD_TIME desc
                """
...
def displayProduct(request,id,product_id):
    ...
    sql = """   SELECT im.image_url
                FROM IMAGE im, PRODUCT pd
                WHERE im.PRODUCT_ID = pd.PRODUCT_ID and im.PRODUCT_ID=:p
            """
    c.execute(sql, {'p':product_id})
    result1 = []
    result1 = c.fetchall()
    picture_of_product = str(result1[0][0])
    ...
    if id==1:
        ...
        for r in result:
            ...
            row = {'sname': sproduct_name,
                'sp_type': spet_type, 'sprice': sprice,'sprid':sproduct_id, 'sproduct_pic':sproduct_pic}
            dict_result.append(row)
        params={'product_id':product_id,'product_name':product_name,'price':price,'description':description,'contact_no':contact_no,'pet_type':pet_type,'age': age,'color':color,'food_habit':food_habit,'gender':gender,'fullname':fullname,'payment_system':payment_system,'ad_time':ad_time,'thana':thana,'district':district,'division':division,'id':id,'similar':dict_result, 'user_of_product':user_of_product, 'picture_of_product':picture_of_product}
    elif id==3:
        cursor = conn.cursor()
        ...
        for r in result:
            ...
            sproduct_pic = r[4]
            row = {'sname': sproduct_name,
                'sprice': sprice,'sc_title': scourse_title,'sprid':sproduct_id, 'sproduct_pic':sproduct_pic}
            dict_result.append(row)
        params={'product_id':product_id,'product_name':product_name,'price':price,'description':description,'contact_no':contact_no,'course_title':course_title,'organization':organization,'fullname':fullname,'payment_system':payment_system,'ad_time':ad_time,'thana':thana,'district':district,'division':division,'id':id,'similar':dict_result, 'user_of_product':user_of_product, 'picture_of_product':picture_of_product}
    elif id==5:
        cursor = conn.cursor()
        ...
        for r in result:
            ...
            sproduct_pic = r[5]
            row = {'sname': sproduct_name,
                'st_gender': stutor_gender, 'sprice': sprice,'sedu_level': seducation_level,'sprid':sproduct_id, 'sproduct_pic':sproduct_pic}
            dict_result.append(row)
        params={'product_id':product_id,'product_name':product_name,'price':price,'description':description,'contact_no':contact_no,'tution_subject':subject,'tutor_gender':t_gender,'time_duration':time,'education_level':edu_level, 'fullname':fullname,'payment_system':payment_system,'ad_time':ad_time,'thana':thana,'district':district,'division':division,'id':id,'similar':dict_result, 'user_of_product':user_of_product, 'picture_of_product':picture_of_product}  
    conn.close()
